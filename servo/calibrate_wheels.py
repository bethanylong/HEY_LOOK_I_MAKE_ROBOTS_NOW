...
def was_accidental_single_step(angle, last_angle, side):
    if last_angle is None:
        # Special case: got a "yes" answer on the first try (too timid)
        return False
...
def calibrate_servo(index=0):
    ...
    if actual_left_bound is not None and actual_right_bound is not None:
        ...
        pca.servo[index].angle = actual_midpoint
    else:
        pca.servo[index].angle = 90
